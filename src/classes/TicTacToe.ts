...
class TicTacToe {
    ...
    invitePlayer = async (newPlayerId: Snowflake, interaction: SelectMenuInteraction) => {
        ...
        this.#latestInvitation = await interaction.message.reply(`_<@${newPlayerId}> You've been invited to a game of Tic-Tac-Toe. Press the checkmark in the message above to accept, or the cross to decline._`);
    }
    ...
    takeTurn = async (v: number, h: number, interaction: ButtonInteraction) => {
        ...
        // diagonals
        victories.push([this.#board[0][0], this.#board[1][1], this.#board[2][2]],
